...
class Home extends CI_Controller
{
    ...
    public function courses()
    {
        ...
        if (isset($_GET['sort_by']) && !empty($_GET['sort_by'])) {
            //check double quote and script text in the search string
            if(preg_match('/"/', $_GET['sort_by']) >= 1 && strpos($_GET['sort_by'],"script") >= 1){
                $this->session->set_flashdata('error_message', site_phrase('such_script_searches_are_not_allowed').'!');
                redirect(site_url('home/courses'), 'refresh');
            }
        }
    ...
    function course_payment(){
        ...
        $query = $this->db->where('user_id', $this->session->userdata('user_id'))->where_in('course_id', $this->session->userdata('cart_items'))->get('enrol');
        if($query->num_rows() > 0 && $this->session->userdata('send_gift_to_id') <= ''){
            $this->session->set_flashdata('error_message', site_phrase('You are already enrolled').'!');
            redirect(site_url('home/shopping_cart'), 'refresh');
        }elseif($this->session->userdata('send_gift_to_id') > 0){
            $gift_to_user_id = $this->session->userdata('send_gift_to_id');
            $query = $this->db->where('user_id', $gift_to_user_id)->where_in('course_id', $this->session->userdata('cart_items'))->get('enrol');
            if($query->num_rows() > 0){
                $this->session->unset_userdata('send_gift_to_id');
                $this->session->set_flashdata('error_message', site_phrase('The user is already enrolled').'!');
                redirect(site_url('home/shopping_cart'), 'refresh');
            }
        }
