          ...
          <div class="wrapper">
			...
			if ($course['is_free_course'] == 1) {
				if (!empty($course['price']))
					$price_to_show = currency($course['price']);
				echo site_phrase('free'); 
			}
            else
            if ($course['discount_flag'] == 1) { 
				$price_to_show = currency($course['discounted_price']);
				echo $price_to_show;
			}
			else {
				$price_to_show = currency($course['price']);
			  ...
			  // PZ
                if ($this->session->userdata('user_login') != 1) {
				  ...
				  if (empty($price_to_show)) {
					...
					echo '<a href="'.$url.'" class="" onclick="handleEnrolledButton()">'.site_phrase('enrol').'></a>';
				  }
				  else {
					echo '<a href="javascript::" class="big-cart-button-'.$course['id'].'>" id = "'.$course['id'].'" onclick="showFreeOrPay(this, `'.$course['title'].'`, `'.$price_to_show.'`)">'.site_phrase('enrol').'</a>';		  
			      }
                }
				?>
                
              <?php else: ?>
                <a href="javascript::" class="big-cart-button-<?php echo $course['id'];?> <?php if(in_array($course['id'], $cart_items)) echo 'addedToCart'; ?>" id = "<?php echo $course['id']; ?>" onclick="handleCartItems(this, undefined, `<?php echo $course['title']; ?>`, `<?php echo $price_to_show; ?>`)">
                  <?php
