        ...
        <div class="col-12">
            ...
            <div class="tab-content" id="lessonTabContent">
                ...
                <?php if (addon_status('certificate')): ?>
					<?php
					// PZ
					$must_pay_for_cerificate = floatval($course_details["price"]) > 0 && empty($this->db->get_where('payment', ['user_id' => $this->session->userdata('user_id'), 'course_id' => $course_id])->row('amount'));
					?>
                    <div class="tab-pane fade" id="certificate" role="tabpanel" aria-labelledby="certificate-tab" style="text-align: center;">
                        ...
                        <div class="alert alert-success" id="certificate-alert-success" role="alert">
                            ...
                            <p><?php
							//PZ
								if ($must_pay_for_cerificate)
									echo get_phrase('Purchase_the_certificate_it_is_a_trusted_way');
								else
									echo get_phrase('you_are_now_eligible_to_download_the_course_completion_certificate').'.';
                        ...
                        <div id="download_certificate_area" style="padding: 15px;">
							...
							// PZ
								if ($must_pay_for_cerificate)
									echo '<a href="javascript::" class="btn btn-primary" onclick="pay_for_course(`'.$course_id.'`);">'.get_phrase('get_certificate').' '.currency($course_details['price']).'</a>';
								else
									echo '<a href="#" target="_blank" class="btn btn-primary" id="certificate_download_btn" disabled>'.get_phrase('get_certificate').'</a>';
							}
							else {
								echo '<a href="#" target="" class="btn btn-primary">'.get_phrase('install_certificate_addon_first').'</a>';
							}
							?>
                        </div>
...
function pay_for_course(course_id)
{
