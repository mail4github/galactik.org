...
$sber_arr = $this->db->get_where('payment_gateways', ['identifier' => 'sber'])->row_array();
$sber_keys = json_decode($sber_arr['keys']);
//var_dump($sber_arr); exit;
...
<div class="container">
	...
	if (!empty($_GET['status'])) {
		echo '
		<div class="alert alert-'.($_GET['status'] == 'charged' ? 'success' : 'danger').'" role="alert">
			<h4 class="alert-heading">'.($_GET['status'] == 'charged' ? get_phrase('congratulations') : get_phrase('oops')).'</h4>
			<p><strong>'.($_GET['status'] == 'charged' ? '' : 'Message from bank: "'.$_GET['message'].'", status: ').' '.$_GET['status'].'</strong></p>
		</div>';
	}
	?>
			...
			<div class="row">
				<div class="col-md-3">
					...
					<?php endif; ?>
						...
						<div class="row payment-gateway paymtech" onclick="selectedPaymentGateway('paymtech')">
							<div class="col-12">
								<img class="tick-icon paymtech-icon" src="<?php echo base_url('assets/payment/tick.png'); ?>">
								<img class="payment-gateway-icon" src="<?php echo base_url('assets/payment/visa_mastercard.svg'); ?>">
							</div>
						</div>
					
					<?php if ($sber_arr['status'] != '0') : ?>
						<div class="row payment-gateway razorpay" onclick="selectedPaymentGateway('razorpay')">
							<div class="col-12 py-3px">
								<h3>Cber</h3>
								<img class="tick-icon razorpay-icon" src="<?php echo base_url('assets/payment/tick.png'); ?>">
				...
				<div class="col-md-8">
					<div class="w-100 d-grid">
						...
						<?php foreach ($this->session->userdata('cart_items') as $cart_item) :
							...
							$description = $description.(empty($description) ? '' : ', ').$course_details['title'];

							// The curses, which are purchased, are packed in one octet number where each course_id separated from each other by '8'
							$course_id = $course_id.(empty($course_id) ? '' : '8').base_convert($course_details['id'], 10, 8);
					...
					<div class="w-100 float-start">
						<form id="paymtech_kz" action="/paymtech_kz_pay.php?method=create" method="post" class="paymtech-form form">
							...
							<input type="hidden" name="amount" value="<?php echo $total_price_of_checking_out; ?>">
							<input type="hidden" name="description" value="<?php echo $description; ?>">
							<input type="hidden" name="course_id" value="<?php echo $course_id; ?>">
							<input type="hidden" name="user_id" value="<?php echo $this->session->userdata('user_id'); ?>">
							<button type="submit" class="payment-button float-end"><?php echo get_phrase('continue'); ?> ...</button>
						</form>
...
<script type="text/javascript">
	function selectedPaymentGateway(gateway) {
		...
		if (gateway == 'paymtech') {
			...
			$(".paymtech").css("border", "2px solid #00D04F");
			$('.paymtech-icon').show();
			$('.paymtech-form').show();

			document.getElementById("paymtech_kz").submit();
		} 
